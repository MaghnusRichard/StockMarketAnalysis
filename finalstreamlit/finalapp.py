...
# train_X_hdfc, test_X_hdfc, train_Y_hdfc, test_Y_hdfc, scaler_target_hdfc = load_and_preprocess_data(hdfc_df)

# # # Load Reliance data
# # reliance_df = pd.read_csv('reliance_merged.csv')
# # train_X_reliance, test_X_reliance, train_Y_reliance, test_Y_reliance, scaler_target_reliance = load_and_preprocess_data(reliance_df)
...
# company = st.sidebar.selectbox('Select Company', ['HDFC', 'Reliance','TCS'])
...
#     dates = hdfc_df['Date'].values
#     sampleInterval = 60
#     sampledDates = dates[::sampleInterval]

#     plt.figure(figsize=(20, 10))
#     plt.plot(dates[:len(train_Y_hdfc)], train_Y_hdfc, label='Actual Train')
#     plt.plot(dates[:len(train_predict_hdfc)], train_predict_hdfc, label='Predicted Train')
#     plt.plot(dates[len(train_Y_hdfc):len(train_Y_hdfc) + len(test_Y_hdfc)], test_Y_hdfc, label='Actual Test')
#     plt.plot(dates[len(train_predict_hdfc):len(train_predict_hdfc) + len(test_predict_hdfc)], test_predict_hdfc, label='Predicted Test')

#     plt.xticks(sampledDates, rotation=90)

#     plt.legend()
#     plt.title('Comparison between actual and predicted open price')
#     plt.xlabel('Date')
...
#     fig_tcs.update_layout(title='Comparison between actual open price and predicted open price for Reliance', xaxis_title='Date', yaxis_title='Open Price')
...
# Dropdown to select company
company = st.sidebar.selectbox('Select Company', ['HDFC', 'Reliance', 'TCS', 'Wipro', 'Infosys'])
